...
async function updateToQlEnvConfig({ name, value, desc }, updateEnvValue) {
  ...
  if (!item) {
    ...
    if (list.code !== 200) {
      logger.log('updateToQlEnvConfig', list.message);
      return;
    }
  ...
  if (item) {
    if (item.value.includes(value)) {
      logger.info('updateToQlEnvConfig', `${cyan(name)} is already ${gray(value)}`);
      return;
    }
  ...
  logger.log(`${item ? green('更新') : magenta('新增')}QL环境变量[${name}]`, isSuccess ? '成功' : r.data.message || '失败');
...
async function updateEnvConfigFile({ name, value, desc }, updateEnvValue, filePath) {
  ...
  logger.log(`[${cyan(filePath)}]${isExist ? green('更新') : magenta('新增')}环境变量`, `${green(name)}="${gray(value)}"`);
  return true;
}
